...
class DatabaseService {
  ...
  Future<int> getSelectedRopeTypeId({required String compareType,required String compareCore,required String compareConstruction}) async{
  ...
  getAllRopesRate() async{
    ...
    List<Map<String,dynamic>> resultList = await _database.query(ropesRateTable);

    resultList.forEach((element) {print(element);});
  ...
  Future<List<ExcelData>> getSlingsTypeFromTempTable() async {
    ...
    try {
      List<Map<String, dynamic>> resultList = await _database.query(
          excelDataTempTable, where: '$type = ?', whereArgs: ['slings']);

      List<ExcelData> slingsExcelDataList = resultList.map((data) =>
          ExcelData.fromJson(data)).toList();
  ...
  Future<int> getRateId({required int compareSelectedRopeTypeId, required String compareDiameter}) async{
  ...
  getAllSlingsRate() async{
    ...
    List<Map<String,dynamic>> resultList = await _database.query(slingsRateTable);
